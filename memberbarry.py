...
OPENAI_STT_ENGINE = "whisper-1"
OPENAI_MODEL = "gpt-3.5-turbo"  # or "gpt-4" if ya got it

# SUMMARY_TOKEN_LIMIT = 200 # 
CONTEXT_TOKEN_LIMIT = 500 # 

class MemberBarry:
    """A wrapper class for interacting with the OpenAI API and retaining context."""  

    def __init__(
            self, system_prompt=None, session_id=None, openai_model=None, stt_engine=None, 
            temp=None, max_tokens=None, db_filename=None):
    ...
    def send_prompt(self, text, system_prompt=None, use_full_context=False, 
        ...
        Args:
            ...
            system_prompt (str): The system prompt to send to the OpenAI API.
            use_full_context (bool): Whether to use the full context in the instance.
        ...
        if system_prompt and use_full_context:
            raise ValueError(
                ...
                system prompt. The stored full context already contains a system 
                prompt. Please remove your system prompt or send an explicit 
                ad-hoc context.""")
        ...
        if use_full_context:
            ...
            for msg in self.running_context:
                # There is debate as to how the system prompt should be handled.
                # Sending it first, last, or just sending system directives as a 
                # standard user prompt. We'll be sending it first here.
                #
                # See: https://community.openai.com/t/the-system-role-how-it-influences-the-chat-behavior/87353
                if msg['role'] == 'system':
    ...
    def add_to_running_context(self, **kwargs):
        ...
        Example:
            add_to_running_context(user="Hello.", assistant="Greetings, flesh bag.")
        """
    ...
    def get_token_count_running_context(self):
        ...
        count = self.get_token_count(" ".join([m['content'] for m in self.running_context]))
    ...
    def get_token_count_full_context(self):
        ...
        return self.get_token_count(" ".join([message['content'] for message in msg]))
